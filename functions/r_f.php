...
function select_adunit_query($campaign_id) {
	...
	while ( $ad_detail = mysql_fetch_array ( $usrres ) ) {
		$add = array (
				'ad_id' => $ad_detail ['adv_id'],
				'width' => $ad_detail ['adv_width'],
				'height' => $ad_detail ['adv_height'] 
		);
...
function launch_campaign_query($q) {
	...
	while ( $campaign_detail = mysql_fetch_array ( $usrres ) ) {
		$add = array (
				'campaign_id' => $campaign_detail ['campaign_id'],
				'priority' => $campaign_detail ['campaign_priority'],
				'type' => $campaign_detail ['campaign_type'],
				'network_id' => $campaign_detail ['campaign_networkid'] 
		);
...
function build_query() {
	...
	if (isset ( $request_settings ['gender'] ) && is_numeric ( $request_settings ['gender'] )) {
		$query_part ['gender'] = "AND (md_campaigns.gender_target=1 OR (c3.targeting_type='gender' AND c3.targeting_code='" . $request_settings ['gender'] . "'))";
	} else {
		$query_part ['gender'] = '';
	}
	
	if (isset ( $request_settings ['income'] ) && is_numeric ( $request_settings ['income'] )) {
		$query_part ['income'] = "AND (md_campaigns.income_target=1 OR (c4.targeting_type='income' AND c4.targeting_code='" . $request_settings ['income'] . "'))";
	} else {
		$query_part ['income'] = '';
	}
	...
	if (isset ( $request_settings ['location'] ) && is_numeric ( $request_settings ['location'] )) {
		$query_part ['location'] = "AND (md_campaigns.location_target=1 OR (c6.targeting_type='location' AND c6.targeting_code='" . $request_settings ['location'] . "'))";
	} else {
		$query_part ['location'] = '';
	}
