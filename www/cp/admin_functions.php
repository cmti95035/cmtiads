...
echo '<p>Server has detected that the required daily cron-job has not not been installed yet. Please note that you will not be able to add impression caps to your campaigns until you install the cron job. <strong>Please note that once you intall the cron job, it will take up to 24 hours for this message to disappear.</strong></p>';
echo '</div> <!-- .notify -->';
echo '</div>';
}
}
...
function load_campaign_income_array($id){
	global $maindb;
	$income_array=array();
	$usrres=mysql_query("select targeting_code from md_campaign_targeting where campaign_id='$id' and targeting_type='income'", $maindb);
	while($targeting_detail=mysql_fetch_array($usrres)){
		array_push($income_array, $targeting_detail['targeting_code']);
	}
	return $income_array;
}
...
function load_campaign_location_array($id){
	global $maindb;
	$location_array=array();
	$usrres=mysql_query("select targeting_code from md_campaign_targeting where campaign_id='$id' and targeting_type='location'", $maindb);
	while($targeting_detail=mysql_fetch_array($usrres)){
		array_push($location_array, $targeting_detail['targeting_code']);
	}
	return $location_array;
}

function load_campaign_age_array($id){
	global $maindb;
	$age_array=array();
	$usrres=mysql_query("select targeting_code from md_campaign_targeting where campaign_id='$id' and targeting_type='age'", $maindb);
	while($targeting_detail=mysql_fetch_array($usrres)){
		array_push($age_array, $targeting_detail['targeting_code']);
	}
	return $age_array;
}
...
if (!isset($data['location_select'])){$data['location_select']='';}
if (!isset($data['age_select'])){$data['age_select']='';}
...
if ($data['location_targeting']==2 && count($data['location_select'])<1){
	global $errormessage;
	$errormessage='Please select the location you want to target.';
	global $editdata;
	$editdata=$data;
	return false;
}

if ($data['age_targeting']==2 && count($data['age_select'])<1){
	global $errormessage;
	$errormessage='Please select the age you want to target.';
	global $editdata;
	$editdata=$data;
	return false;
}
...
$data['location_targeting']=sanitize($data['location_targeting']);
$data['age_targeting']=sanitize($data['age_targeting']);
$data['chroniccondition_targeting']=sanitize($data['chroniccondition_targeting']);


global $maindb;

mysql_query("UPDATE md_campaigns set campaign_type='$data[campaign_type]', campaign_name='$data[campaign_name]', campaign_desc='$data[campaign_desc]', ".
...
// income
if ($data['income_targeting']==2 && is_array($data['income_select'])){
	foreach($data['income_select'] as $income_id){
		add_campaign_targeting($detail, 'income', $income_id);
	}
}
// End income
...
// location
if ($data['location_targeting']==2 && is_array($data['location_select'])){
	foreach($data['location_select'] as $location_id){
		add_campaign_targeting($detail, 'location', $location_id);
	}
}
// End location

// age
if ($data['age_targeting']==2 && is_array($data['age_select'])){
	foreach($data['age_select'] as $age_id){
		add_campaign_targeting($detail, 'age', $age_id);
	}
}
// End age
...
if ($data['income_targeting']==2 && count($data['income_select'])<1){
	global $errormessage;
	$errormessage='Please select income you want to target.';
	global $editdata;
	$editdata=$data;
	return false;
}
...
if ($data['location_targeting']==2 && count($data['location_select'])<1){
	global $errormessage;
	$errormessage='Please select location you want to target.';
	global $editdata;
	$editdata=$data;
	return false;
}
...
if ($data['age_targeting']==2 && count($data['age_select'])<1){
	global $errormessage;
	$errormessage='Please select age you want to target.';
	global $editdata;
	$editdata=$data;
	return false;
}
...
mysql_query("INSERT INTO md_campaigns (campaign_owner, campaign_status, campaign_type, campaign_name, campaign_desc, ".
	...
	"'$data[gender_targeting]', '$data[income_targeting]', '$data[interest_targeting]', '$data[location_targeting]', '$data[age_targeting]', '$data[chroniccondition_targeting]')", $maindb);
...
if ($data['income_targeting']==2 && is_array($data['income_select'])){
	foreach($data['income_select'] as $income_id){
		add_campaign_targeting($created_campaign_id, 'income', $income_id);
	}
...
if ($data['location_targeting']==2 && is_array($data['location_select'])){
	foreach($data['location_select'] as $location_id){
		add_campaign_targeting($created_campaign_id, 'location', $location_id);
	}
}
// End location
...
if ($data['age_targeting']==2 && is_array($data['age_select'])){
	foreach($data['age_select'] as $age_id){
		add_campaign_targeting($created_campaign_id, 'age', $age_id);
	}
}
// End age
...
function list_income_campaign($selected){
	if (!is_array($selected)){$selected=array();}
	global $maindb;
	$x=0;
	$usrres=mysql_query("select * from md_incomes", $maindb);
	//continue debug here
	while($income_detail=mysql_fetch_array($usrres)){

		if ($x==1){ echo '<tr style="background-color:#F2F2F2;">'; } else { echo '<tr>'; }
		echo '<td width="45%"><div>';
		if (in_array($income_detail['income_id'], $selected)) { $selectedhtml='checked="checked"';} else {$selectedhtml='';}
		echo '<input '.$selectedhtml.' value="'.$income_detail['income_id'].'" name="income_select[ ]" type="checkbox" /> '.$income_detail['income_name'].'';
		echo '</div></td>';
		echo '</tr>';
...
function list_location_campaign($selected){
	if (!is_array($selected)){$selected=array();}
	global $maindb;
	$x=0;
	$usrres=mysql_query("select * from md_locations", $maindb);
	//continue debug here
	while($location_detail=mysql_fetch_array($usrres)){

		if ($x==1){ echo '<tr style="background-color:#F2F2F2;">'; } else { echo '<tr>'; }
		echo '<td width="45%"><div>';
		if (in_array($location_detail['location_id'], $selected)) { $selectedhtml='checked="checked"';} else {$selectedhtml='';}
		echo '<input '.$selectedhtml.' value="'.$location_detail['location_id'].'" name="location_select[ ]" type="checkbox" /> '.$location_detail['location_name'].'';
		echo '</div></td>';
		echo '</tr>';
...
function list_age_campaign($selected){
	if (!is_array($selected)){$selected=array();}
	global $maindb;
	$x=0;
	$usrres=mysql_query("select * from md_ages", $maindb);
	//continue debug here
	while($age_detail=mysql_fetch_array($usrres)){

		if ($x==1){ echo '<tr style="background-color:#F2F2F2;">'; } else { echo '<tr>'; }
		echo '<td width="45%"><div>';
		if (in_array($age_detail['age_id'], $selected)) { $selectedhtml='checked="checked"';} else {$selectedhtml='';}
		echo '<input '.$selectedhtml.' value="'.$age_detail['age_id'].'" name="age_select[ ]" type="checkbox" /> '.$age_detail['age_name'].'';
		echo '</div></td>';
		echo '</tr>';
...
foreach ($traffic_request->targeting->income_targeting->value as $income_targeting_code){
	/*Add income Targeting*/
...
foreach ($traffic_request->targeting->location_targeting->value as $location_targeting_code){
	/*Add location Targeting*/
	add_trafficrequest_targeting($created_request_id, 'location', $location_targeting_code);
}

foreach ($traffic_request->targeting->age_targeting->value as $age_targeting_code){
	/*Add age Targeting*/
	add_trafficrequest_targeting($created_request_id, 'age', $age_targeting_code);
}
...
// income Targeting
$income_array=load_tr_income_array($request_id);

if (count($income_array)>0){
	$campaigndata['income_targeting']=2;
	$campaigndata['income_select']=$income_array;
}
else {
	$campaigndata['income_targeting']=1;
}
...
// location Targeting
$location_array=load_tr_location_array($request_id);

if (count($location_array)>0){
	$campaigndata['location_targeting']=2;
	$campaigndata['location_select']=$location_array;
}
else {
	$campaigndata['location_targeting']=1;
}

// location Targeting End

// age Targeting
$age_array=load_tr_age_array($request_id);

if (count($age_array)>0){
	$campaigndata['age_targeting']=2;
	$campaigndata['age_select']=$age_array;
}
else {
	$campaigndata['age_targeting']=1;
}
...
function load_tr_location_array($id){
	global $maindb;
	$location_array=array();
	$usrres=mysql_query("select parameter_value from md_trafficrequests_parameters where request_id='$id' and parameter_id='location'", $maindb);
	while($targeting_detail=mysql_fetch_array($usrres)){
		array_push($location_array, $targeting_detail['parameter_value']);
	}
	return $location_array;
}

function load_tr_age_array($id){
	global $maindb;
	$age_array=array();
	$usrres=mysql_query("select parameter_value from md_trafficrequests_parameters where request_id='$id' and parameter_id='age'", $maindb);
	while($targeting_detail=mysql_fetch_array($usrres)){
		array_push($age_array, $targeting_detail['parameter_value']);
	}
	return $age_array;
}
